...
import type { CreateInboundPlanInput } from '@modules/InboundPlan/Domain/Types/InboundPlanQuery';
...
vi.mock('@modules/InboundPlan/Application/Commands/UseInboundPlanMutations', () => ({
...
function renderPage() {
  return render(
    <MemoryRouter>
      <InboundPlanCreatePage />
    </MemoryRouter>,
  );
}

describe('InboundPlanCreatePage Excel import (IFB-03)', () => {
  beforeEach(() => {
  ...
  it('commits a clean preview with the page header and navigates to the new plan', async () => {
    ...
    expect(h.navigate).toHaveBeenCalledWith(ROUTES.INBOUND_PLAN.DETAIL('inbound-plan-1'));
  });
});

describe('InboundPlanCreatePage warehouse search (IFB-16)', () => {
  afterEach(() => cleanup());
