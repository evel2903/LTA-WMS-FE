...
const h = vi.hoisted(() => ({ startReceivingSession: vi.fn() }));

vi.mock('@modules/InboundReceiving/Infrastructure/Repositories/InboundReceivingRepositoryInstance', () => ({
  inboundReceivingRepository: { startReceivingSession: h.startReceivingSession },
}));
...
describe('useInboundReceivingMutations cross-invalidate (ipr-01 AC12c)', () => {
  it("invalidates the Receiving namespace AND the Plan module's ENTIRE namespace (detail + list) after startReceivingSession succeeds", async () => {
    ...
    client.setQueryData(inboundReceivingQueryKeys.operationalState('inbound-plan-1'), { stale: true });
    client.setQueryData(inboundPlanQueryKeys.detail('inbound-plan-1'), { id: 'inbound-plan-1', status: 'Planned' });
    client.setQueryData(inboundPlanQueryKeys.list(), { items: [{ id: 'inbound-plan-1', status: 'Planned' }] });
    ...
    expect(
      client.getQueryState(inboundReceivingQueryKeys.operationalState('inbound-plan-1'))?.isInvalidated,
    ).toBe(true);
    ...
    expect(client.getQueryState(inboundPlanQueryKeys.detail('inbound-plan-1'))?.isInvalidated).toBe(true);
  ...
  it('does not touch the Plan cache when no planId is available (e.g. hook mounted before the route param resolves)', async () => {
    ...
    const { result } = renderHook(() => useInboundReceivingMutations(null), { wrapper: wrapperFor(client) });
    ...
    expect(client.getQueryState(inboundPlanQueryKeys.detail('inbound-plan-1'))?.isInvalidated).toBe(false);
  });
});
